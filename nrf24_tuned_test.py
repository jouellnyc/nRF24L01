...
_RESPONDER_SEND_DELAY = const(100)
...
def initiator():
    ...
    # Back to your best working settings
    ...
    print("NRF24L01 detected:", nrf.reg_read(0x10) == 0x73)
    ...
    while num_successes < num_needed and num_failures < num_needed:
        ...
        nrf.stop_listening()
        utime.sleep_ms(2)  # Small delay to ensure mode switch completes
        ...
        nrf.start_listening()
        utime.sleep_ms(5)  # Small delay to ensure mode switch completes
        ...
        if timeout:
            ...
            print("NRF status:", hex(nrf.reg_read(0x07)))  # STATUS register
            print("RX FIFO empty:", not nrf.any())
            num_failures += 1
        else:
            # recv packet
...
def responder():
    ...
    while True:
        if nrf.any():
            ...
            # Send response (only once, with error handling)
            try:
                print(f"About to send response: {millis}")
